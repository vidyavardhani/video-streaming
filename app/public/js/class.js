(() => {
  ...
  const elements = {
    ...
    screenShareAudio: document.getElementById('screen-share-audio'),
    ...
    pollAllowMultiple: document.getElementById('poll-multiple'),
    ...
    recordingHistory: document.getElementById('recording-history'),
  ...
  const hideConnectionBanner = () => {
    ...
    elements.connectionBanner.classList.remove('visible', 'warn', 'error', 'info');
    if (state.connectionBannerTimer) {
      clearTimeout(state.connectionBannerTimer);
      state.connectionBannerTimer = null;
    }
  };

  const showConnectionBanner = (message, tone = 'info', timeout = 4000) => {
    if (!elements.connectionBanner) return;
    ...
    elements.connectionBanner.dataset.tone = tone;
    elements.connectionBanner.classList.toggle('warn', tone === 'warn');
    elements.connectionBanner.classList.toggle('error', tone === 'error');
    elements.connectionBanner.classList.toggle('info', tone === 'info');
    if (state.connectionBannerTimer) {
      clearTimeout(state.connectionBannerTimer);
    }
    if (timeout) {
      state.connectionBannerTimer = setTimeout(() => {
  ...
  const updateRecordingStatus = () => {
    ...
    if (elements.recordingsList) {
      const recordings = (state.classInfo?.recordings || []).slice().sort((a, b) => {
        const left = new Date(b.startedAt || b.endedAt || 0).getTime();
        const right = new Date(a.startedAt || a.endedAt || 0).getTime();
        return left - right;
      });
      elements.recordingsList.innerHTML = '';
      if (recordings.length) {
        elements.recordingsEmpty?.classList.add('hidden');
        elements.recordingHistory?.classList.remove('hidden');
        recordings.forEach((entry) => {
          const li = document.createElement('li');
          li.className = 'recording-item';
          const title = document.createElement('strong');
          title.textContent = state.classInfo?.title || 'Class recording';
          const meta = document.createElement('div');
          meta.className = 'recording-meta';
          const started = formatTime(entry.startedAt);
          const ended = formatTime(entry.endedAt);
          meta.textContent = `${started} – ${ended}`;
          ...
          li.appendChild(meta);
          li.appendChild(link);
          elements.recordingsList.appendChild(li);
        });
      } else {
        elements.recordingHistory?.classList.add('hidden');
        elements.recordingsEmpty?.classList.remove('hidden');
      }
    }
  };
  ...
  const renderPolls = () => {
    ...
    if (hasActive) {
      ...
      const allowMultiple = !!state.activePoll.allowMultiple;
      ...
      const currentSelection = Array.isArray(currentVote?.optionIds)
        ? currentVote.optionIds
        : currentVote?.optionId
        ? [currentVote.optionId]
        : [];
      state.pollSelection = new Set(currentSelection);
      ...
      state.activePoll.options.forEach((option) => {
        ...
        if (!state.isHost && state.admitted) {
          ...
          const selected = state.pollSelection.has(option.id);
          voteButton.textContent = selected ? (allowMultiple ? 'Selected' : 'Voted') : allowMultiple ? 'Select' : 'Vote';
          voteButton.setAttribute('aria-pressed', selected ? 'true' : 'false');
          if (selected) {
            voteButton.classList.add('primary');
          }
          voteButton.addEventListener('click', () => {
            if (allowMultiple) {
              if (state.pollSelection.has(option.id)) {
                state.pollSelection.delete(option.id);
              } else {
                state.pollSelection.add(option.id);
              }
              if (!state.pollSelection.size) {
                state.pollSelection.add(option.id);
              }
              submitPollVote(Array.from(state.pollSelection), true);
            } else {
              state.pollSelection = new Set([option.id]);
              submitPollVote([option.id], false);
            }
            renderPolls();
          });
  ...
  const submitPollVote = async (selections, allowMultiple) => {
    if (!Array.isArray(selections) || !selections.length) return;
    try {
      const payload = { joinToken: state.joinToken };
      if (allowMultiple) {
        payload.optionIds = selections;
      } else {
        payload.optionId = selections[0];
      }
  ...
  const renderParticipants = () => {
    ...
    participants.forEach((participant) => {
      ...
      const allowed = participant.allowedToSpeakAt;
      li.classList.toggle('highlight', !!allowed);
      strong.classList.toggle('active-audio', mediaState.audio !== false);
      ...
      if (state.isHost) {
        ...
        toggleAudio.textContent = mediaState.audio === false ? 'Allow mic' : 'Mute';
        ...
        if (mediaState.video !== false) {
          const hideVideo = document.createElement('button');
          hideVideo.type = 'button';
          hideVideo.className = 'ghost small';
          hideVideo.textContent = 'Hide video';
          hideVideo.addEventListener('click', () => sendMediaControl(participant.token, { video: false }));
          actions.appendChild(hideVideo);
        }
  ...
  const applyZoom = () => {
    const scale = Math.min(Math.max(state.zoom, 1), 2.5);
    if (elements.primaryVideo) {
      elements.primaryVideo.style.transform = `scale(${scale})`;
    }
    if (elements.primaryContainer) {
      elements.primaryContainer.dataset.zoom = scale.toFixed(2);
    }
  ...
  const changeZoom = (delta) => {
    state.zoom = Math.min(2.5, Math.max(1, +(state.zoom + delta).toFixed(2)));
    applyZoom();
  };
  ...
  const setupPreview = async () => {
    ...
    try {
      const stream = await ensureLocalMedia({ audio: true, video: true });
      state.previewReady = true;
      if (stream && elements.previewVideo) {
  ...
  const loadClass = async () => {
    ...
    state.classInfo.recordings = state.classInfo.recordings || [];
  ...
  const connectSocket = () => {
    ...
    state.socket.on('disconnect', (reason) => {
      if (reason !== 'io client disconnect') {
        showConnectionBanner('Connection lost. Trying to reconnect…', 'warn', 0);
      }
    });
    state.socket.io.on('reconnect_attempt', () => {
      showConnectionBanner('Reconnecting…', 'warn', 0);
    });
    state.socket.on('reconnect', () => {
      showConnectionBanner('Back online', 'info', 2500);
    });
    state.socket.on('connect_error', () => {
      showConnectionBanner('Network unstable. Retrying…', 'warn', 0);
    });
    ...
    state.socket.on('connect', () => {
      ...
      state.keepAliveTimer = window.setInterval(() => {
        if (state.socket?.connected) {
          state.socket.emit('session:ping', null, () => {});
        }
      }, 15000);
      state.socket.emit(
        'session:join',
    ...
    state.socket.on('participant:media', ({ joinToken, mediaState, allowedToSpeakAt }) => {
      applyMediaState(joinToken, mediaState);
      updateParticipantState(joinToken, { mediaState, allowedToSpeakAt });
      renderParticipants();
    });
    ...
    state.socket.on('host:media', (mediaState) => {
      void applyHostMediaState(mediaState);
    });
    ...
    state.socket.on('poll:voted', ({ pollId, responses, options, allowMultiple }) => {
      ...
      if (typeof allowMultiple === 'boolean') {
        state.activePoll.allowMultiple = allowMultiple;
      }
      renderPolls();
    });
    ...
    state.socket.on('qna:answered', (entry) => {
      ...
      state.socket.on('recording:status', (payload) => {
        ...
        if (Array.isArray(payload?.recordings)) {
          state.classInfo.recordings = payload.recordings;
        }
        updateRecordingStatus();
      });
  ...
  const createPeerConnection = (targetToken, initiator = false) => {
    ...
    if (state.localStream && (state.isHost || targetToken === 'host')) {
      state.localStream.getTracks().forEach((track) => pc.addTrack(track, state.localStream));
    }
    if (state.isHost && state.screenStream) {
      state.screenSenders = state.screenSenders.filter((entry) => entry.token !== targetToken);
      const senders = state.screenStream.getTracks().map((track) => pc.addTrack(track, state.screenStream));
      state.screenSenders.push({ pc, token: targetToken, senders });
    }
    ...
    pc.onconnectionstatechange = () => {
      if (pc.connectionState === 'connected' && targetToken === 'host' && !state.isHost) {
        showConnectionBanner('Connected to host', 'info', 2000);
      }
      if (['disconnected', 'failed'].includes(pc.connectionState) && targetToken === 'host' && !state.isHost) {
        showConnectionBanner('Connection unstable. Reconnecting…', 'warn', 0);
      }
  ...
  const publishLocalTracks = ({ renegotiate: shouldRenegotiate = true } = {}) => {
    if (!state.localStream) return;
    state.peers.forEach((pc, token) => {
      const shouldSend = state.isHost || token === 'host';
      ...
      if (!state.isHost && token === 'host' && shouldRenegotiate) {
        renegotiate(pc, token);
      }
    });
  };

  const ensureLocalMedia = async ({ audio = false, video = false } = {}) => {
    const hasAudio = !!state.localStream?.getAudioTracks()?.length;
    const hasVideo = !!state.localStream?.getVideoTracks()?.length;
    const needsAudio = audio && !hasAudio;
    const needsVideo = video && !hasVideo;
    ...
    try {
      ...
      publishLocalTracks({ renegotiate: true });
      syncTrackButtons();
      return state.localStream;
    } catch (error) {
  ...
  elements.endButton?.addEventListener('click', async () => {
    if (!window.confirm('Are you sure you want to end session for all?')) {
      return;
    }
  ...
  elements.pollCreateForm?.addEventListener('submit', async (event) => {
    ...
    const res = await fetch(`/classes/${classCode}/polls`, {
      ...
      body: JSON.stringify({
        question,
        options,
        allowMultiple: !!elements.pollAllowMultiple?.checked
      })
    });
    if (!res.ok) {
      const error = await res.json().catch(() => ({}));
  ...
  const applyHostMediaState = async (media) => {
    if (!media) return;
    const wantsAudio = media.audio === true;
    const wantsVideo = media.video === true;

    if (
      (wantsAudio && !state.localStream?.getAudioTracks()?.length) ||
      (wantsVideo && !state.localStream?.getVideoTracks()?.length)
    ) {
      await ensureLocalMedia({ audio: wantsAudio, video: wantsVideo });
    }
  ...
  const stopScreenShare = () => {
    ...
    state.screenSenders.forEach(({ pc, senders, token }) => {
      try {
        (senders || []).forEach((trackSender) => pc.removeTrack(trackSender));
        renegotiate(pc, token);
      } catch (error) {
  ...
  const updateScreenShareAudioToggle = () => {
    if (!elements.screenShareAudio) return;
    elements.screenShareAudio.classList.toggle('hidden', !state.isHost);
    elements.screenShareAudio.setAttribute('aria-pressed', state.screenShareWithAudio ? 'true' : 'false');
    elements.screenShareAudio.textContent = state.screenShareWithAudio
      ? 'System audio on'
      : 'System audio off';
  };
  ...
  elements.screenShareBtn?.addEventListener('click', async () => {
    ...
    try {
      const displayStream = await navigator.mediaDevices.getDisplayMedia({
        video: { frameRate: 30 },
        audio: state.screenShareWithAudio
      ...
      state.peers.forEach((pc, token) => {
        try {
          const senders = tracks.map((track) => pc.addTrack(track, displayStream));
          state.screenSenders.push({ pc, token, senders });
          renegotiate(pc, token);
        } catch (error) {
          console.error('Screen share publish error', error);
        }
      });
      if (elements.screenShareBtn) {
  ...
  elements.screenShareAudio?.addEventListener('click', () => {
    if (!state.isHost) return;
    state.screenShareWithAudio = !state.screenShareWithAudio;
    updateScreenShareAudioToggle();
  });

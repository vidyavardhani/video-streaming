...
module.exports = (io) => {
  ...
  io.on('connection', (socket) => {
          ...
          if (index !== -1) {
            ...
            if (returning) {
              ...
              await pushSystemMessage(
                klass,
                `${participantEntry.displayName} rejoined the class`,
                { event: 'join', token: participantEntry.token, returning: true }
              );
            }
          }
        }
    ...
    socket.on('session:ping', (payload, callback = () => {}) => {
    ...
    socket.on('media:control', async ({ targetToken, audio, video }, callback = () => {}) => {
      ...
      try {
        ...
        if (typeof audio === 'boolean') {
          participant.mediaState = participant.mediaState || {};
          participant.mediaState.audio = audio;
          participant.allowedToSpeakAt = audio ? new Date() : null;
        }
    ...
    socket.on('disconnect', async () => {
      ...
      try {
        ...
        if (participant) {
          ...
          if (!participant.expelledAt) {
            await pushSystemMessage(klass, `${participant.displayName} left the class`, {
              event: 'leave',
              token
            });
          }

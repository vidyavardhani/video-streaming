...
const publicClassShape = (klass) => {
  ...
  return {
    ...
    recordings: (klass.recordings || []).map((entry) => ({
      url: entry.url,
...
exports.admit = async (req, res) => {
  ...
  try {
    ...
    await klass.save();
    await pushSystemMessage(klass, `${participant.displayName} joined the class`, {
      event: 'join',
...
exports.remove = async (req, res) => {
  ...
  try {
    ...
    await klass.save();
    if (participantIndex !== -1 && removed?.displayName) {
      await pushSystemMessage(
        klass,
        `${removed.displayName} was removed by ${req.user.name || 'the host'}`,
        { event: 'remove', token: removed.token }
      );
    }
